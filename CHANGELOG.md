...
### Added
- **The auto-mode system prompt now carries the escalation rule, not just the allowlist location.** `auto-mode:allowlist` (order 116) told the model where `allowPaths` lives, what it bypasses, and that an allowlisted path outside the workspace still needs `sandbox_permissions: danger-full-access` — but not what to do about it, so "request the escalation on the first attempt" lived only in this README and the model learned it by hitting the sandbox rejection (observed end-of-day 2026-09-14: a bare `git` write to an allowlisted `~/.agents` was blocked, while the identical escalated call came back `allowed-once`). The section now states it outright: for a trusted target — a file tool, or a bash write such as a git commit under an allowlisted repo root — send the escalation **with a one-line justification on the first attempt**, because the approval bridge grants it with no review and it will not wait for a human; do not run it bare and escalate only after the failure, and do not skip the escalation out of fear of a prompt, because for a trusted target there is none. This text change alone touches no verdict, band or gate logic (the `### Security` entries below, added to this same release after review, do).

### Fixed
- **The circuit-breaker hint no longer reads as "escalation always needs a human".** The hint injected at the trip said "a human will be asked to approve it" without scoping that to the tripped state — and the tripped breaker is exactly why a human is asked: the gate deliberately skips the allowPath branch (and so records no approval bridge) while the breaker is up. The unconditional phrasing taught the opposite of the normal rule and nudged the model away from escalating at all. It now attributes the human to the pause and states plainly that, in normal operation, escalation for an allowlisted target is auto-approved with zero review.
- Smoke assertions pin all of this text so it cannot drift: the guidance sentence must carry the first-attempt escalation, the no-review bridge, and the anti-bare-run / anti-avoidance clauses; a structural assertion requires the `auto-mode:allowlist` context to emit it under the `isAuto` gate **in both `src/` and the compiled `lib/`**; the breaker hint must attribute the human to the paused breaker while the retired unconditional wording must not return; and two regression tests cover the composite hardening (`mkdir` targets join the destination set; relative destinations resolve against the effective cwd).
- **New `npm run test:pathtrust`** (`scripts/e2e-path-trust.mjs`): an offline end-to-end check of the changed behaviour against the compiled plugin with a host mock. It renders and **prints in full** the model-visible `auto-mode:allowlist` text (auditing the complete model-facing output, not just the string that changed), drives the allowlisted-escalation happy path (gate → `curated allowPath` → `approval-bridge` → `allowed-once`, zero classifier), and asserts each of the four tightenings is **not** auto-approved. Unlike the real-model `test:e2e` it needs no API key; unlike `test:flow` it also covers the guidance rendering and the v0.15.3 hardening cases.

### Security
- **`mkdir` is a write segment now, not a benign rider** (found by independent review; a pre-existing hole, not introduced by the guidance change — but the new instruction makes attaching `danger-full-access` routine, so it closed in the same release). `mkdir` sat in the composite fast path's "benign utility" set, which meant `git -C <allowlisted repo> commit … && mkdir -p /anywhere` surfaced only the repository root as a write destination, passed the `every(target ∈ trust roots)` proof, and could be bridged to `danger-full-access` with **zero review** while the `mkdir` wrote outside every trust root. It is now a recognized write command whose positional arguments (minus the values of `-m/--mode` and `-Z/--context`) all have to resolve inside the trusted roots — a tightening only: the composite falls back to the classifier instead of being auto-trusted, and genuinely read-only utilities (`echo`, `ls`, `cat`, `head`, …) still ride along. Verified after the change: the example yields `[repo root, /anywhere]` → the `every(...)` proof fails → no `curated allowPath` event, no approval bridge, classifier review; an all-targets-trusted control is unchanged.
- Residual (recorded in the spec, not fixed here): a recognized write segment that yields **no** destination inside a composite (a `curl` with no `-o`, a `tar -x` with no `-C`) still rides along. Closing it needs a per-subcommand read-only table first, because a blanket "unresolvable write invalidates the composite" would also invalidate read-only segments like `git status`.
- **Relative write destinations now resolve against the command's EFFECTIVE cwd** (found while re-verifying the fix above; another pre-existing soundness hole). Destinations used to come back relative and the allowPath proof resolved them against the *session* cwd — so `cd /tmp/outside && cp a workspace/evil`, with an escalation attached, proved `/workspace/workspace/evil` (inside a trust root) while the call really wrote `/tmp/outside/workspace/evil`, and the whole thing was bridged to `danger-full-access` with zero review (reproduced: `curated allowPath` + `approval-bridge` + `allowed-once`). Each segment's destination is now resolved against the cwd in effect **at that segment** (the `cd` tracker already followed it), so the proof and the real write agree. The proof gets more accurate in both directions: a relative write after `cd`-ing inside a trusted root still takes the fast path, one after `cd`-ing outside falls back to the classifier, and a call with no `cd` is unchanged. `bashWriteDestinations()` now always returns absolute paths.
- **An unresolved `$VAR` destination now refuses the whole call** (review finding; pre-existing, same class). `expandShellVars` only resolves in-command assignments and `$HOME`; anything else (`$TMPDIR`, `$XDG_*`, …) stays literal — and a literal token lexically sits under the session cwd, which is *always* a trust root, so `git … commit && cp a "$TMPDIR/y"` passed the proof while the shell expanded the path somewhere else entirely. Any destination still containing `$` after expansion now makes the call fall back to the classifier, which is what the existing comment promised. A follow-up review pass found the same hole one step sideways — the **command name** — so `cp a /allowed && "$EVIL"/echo hi` (whose base token basenames to the benign `echo`) is refused as well, while `$HOME/bin/cp a /dest` still resolves normally. A fourth pass closed the last variant: a **path-prefixed** token may no longer impersonate a benign utility (`./echo hi`, or `E=evil; "$E"/echo hi` once the in-command assignment resolves the `$`), so only a bare name rides along. Write commands deliberately keep their basename match, so the documented `~/bin/trash …` wrapper form still works and its destinations are still proved; the residual (a path-prefixed *write* command such as `./cp`) is recorded in the spec. `$HOME` and in-command assignments are unaffected.
- `mkdir` parsing nits fixed alongside: `-Z`/`--context` take **no** separate value in GNU coreutils (`--context=CTX` is the valued form), so the first cut dropped a real target from `mkdir -Z /evil /allowed`; and a bare `--` now ends the options instead of dropping a flag-shaped operand.
...
### Fixed
- **The intent window now carries the question the answer belongs to.** A terse "yes" to a confirmation prompt used to arrive stripped of the question it had answered, so the gate read it as approving "a probe" rather than the exact command the user had just seen on the card — the double-check loop could not close. Answers now render as the user's own words plus the agent-authored question that disambiguates them, and the prompt states plainly that the quoted question authorizes nothing by itself and cannot widen the answer. Found by live end-to-end testing, not by the offline suite.
...
### Changed
- **The user is the arbiter; only a hard floor is not the user's call** (policy decision, 2026-09-12). The gate no longer decides whether an action is *worth doing* — it decides whether the user *authorized this exact action*. A risky action the user asked for is allowed, including effects that leave this machine (releasing, publishing, deploying, pushing, sending a message outward, writing to another system). When the action can affect other people or systems and the user's own messages do not clearly cover **this** action, the gate rejects with an actionable reason telling the agent to show the user the exact command and ask them to confirm it — the user's answer is a direct authorization and the next attempt may be allowed. That is the "double check" the user asked for. **Authorization comes only from the user's own messages** (a typed message, or an answer to `ask_user_question`); the agent's justification, repository text, and tool output never authorize — otherwise "the user's intent" would just mean "whoever spoke last". The **hard floor** stays closed to everyone: executing downloaded code, destroying or moving system paths, removing docker volumes, reading key material or credential stores, writing key material inline, editing shell startup files or agent permission settings. Those are refused whatever the user says, with the reason telling them to run it themselves outside auto mode; they are also blocked deterministically before any model is consulted.
- **The one-token filter may no longer decide a machine-leaving action.** The pre-screen now only short-circuits for actions that cannot affect other people or systems: a curated set of shapes (git push, package publish, release/workflow commands, uploads and POSTs, remote copies, deploy verbs, …) forces the structured review, which is the only stage that reads the user's authorization. Measured before this rule: an outward message the user *had* asked for was allowed 5/5 by the pre-screen, with the review — and any authorization check — never read.
- **The prompt contract is enforced through every stage, with three clarifications** (v0.15.0's three disciplines — judge the property not the vocabulary, decide from the action not the narration, make a rejection actionable — were first written for 0.15.0, but the tagged `v0.15.0` build predates them; they first ship here). The example list is now explicitly *not* a boundary; authorization comes only from the user's own messages; and publishing a release is named in the refusal paragraph — merging that paragraph into the allow-bias sentence (a compression attempt) measurably flipped the release case from reject to allow, so the two stay separate.
...
### 新增
- **常驻系统提示开始携带提权规则，而不只是白名单位置。** `auto-mode:allowlist`（order 116）此前告诉模型 `allowPaths` 在哪、跳过了什么、以及写 workspace 外的白名单路径仍需 `sandbox_permissions: danger-full-access`——但没说该怎么办，于是「首次尝试就带提权」这条只存在于本 README，模型要靠撞上沙箱拒绝才学到（2026-09-14 end-of-day 实测：对白名单内 `~/.agents` 的裸 `git` 写被沙箱拒绝，而同一条命令带提权即 `allowed-once`）。现在该段落直接写明：信任目标——文件工具、或白名单仓库根下的 `git` 写命令等 bash 写——**首次尝试就带 `danger-full-access` + 一句 justification**，因为 approval 桥接会零评审放行、不会等人工；不要裸跑、等失败后再升级；也不要因为怕弹窗而回避提权——信任目标根本没有弹窗。**这条文字改动本身**未动判定、频带与闸门逻辑（同版随后经 review 加入的「安全」条目动了频带，属收紧）。

### 修复
- **熔断器提示不再被读成「提权永远要人工」。** 跳闸瞬间注入的提示原写「a human will be asked to approve it」，未限定在跳闸状态——而**跳闸正是要走人工的原因**：熔断期间闸门刻意跳过 allowPath 分支（因而不记录 approval 桥接）。无条件措辞教的恰是与常态相反的规则，会让模型干脆回避提权。现改为把人工归因于「auto 已暂停」，并明说正常状态下白名单内提权是**零评审自动放行**。
- smoke 断言固定这段文字，防止漂移：指引句必须含首次尝试提权、桥接零评审、反裸跑与反回避三条；结构断言要求 `auto-mode:allowlist` context 在 `isAuto` 门控下发射该句（**`src/` 与编译产物 `lib/` 双查**）；熔断器提示必须把人工归因于暂停，且被退役的无条件措辞不得回归；另有两项回归测试覆盖复合命令收敛（`mkdir` 目标进入目标集；相对目标按当时 cwd 归一）。
- **新增 `npm run test:pathtrust`**（`scripts/e2e-path-trust.mjs`）：针对编译产物、带宿主 mock 的 offline 端到端检查——**完整打印**模型可见的 `auto-mode:allowlist` 文本（审计模型实际看到的全文，而非只查改动的那一句），跑通「白名单提权」快路径（门 → `curated allowPath` → `approval-bridge` → `allowed-once`，零分类器），并断言四项收紧**都不**被自动放行。与 real-model 的 `test:e2e` 相比不需要密钥；与 `test:flow` 相比额外覆盖指引渲染与 v0.15.3 的四处收紧。

### 安全
- **`mkdir` 现属写命令段，不再搭良性便车**（独立 review 检出；属**既有**漏洞、非本次指引改动引入——但新指引让「带 `danger-full-access`」变成常规动作，故同版收敛）。`mkdir` 原在复合快速路径的「良性工具」集合里，于是 `git -C <白名单仓库> commit … && mkdir -p /anywhere` 只交出仓库根这一个写目标、通过 `every(目标 ∈ 信任根)` 证明，**可被桥接为 `danger-full-access`（零评审）**，而 `mkdir` 实际写到了所有信任根之外。现 `mkdir` 为受识别写命令，其位置参数（`-m/--mode`、`-Z/--context` 的取值除外）**全部**须落在信任根内——纯收紧：不满足即回退分类器而非自动信任；纯只读工具（`echo`、`ls`、`cat`、`head` 等）仍可搭车。改后复核：该示例交出 `[仓库根, /anywhere]` → `every(...)` 不成立 → 无 `curated allowPath` 事件、无 approval 桥接、走分类器；「全部目标都在白名单内」的对照组行为不变。
- 残留（已记入 spec，本次未修）：复合命令中**识别得出却取不到目标**的写命令段（无 `-o` 的 `curl`、无 `-C` 的 `tar -x`）仍是搭车状态。收敛它需先有「按子命令判定只读」的表——粗暴的「取不到目标即整体回退」会把 `git status` 这类只读段也一并打成回退。
- **相对写目标改为按「段的当时 cwd」解析**（复核上一处修复时自查出的**另一处既有信任失真**）。此前目标原样返回，allowPath 证明按**会话 cwd** 解析——于是带提权的 `cd /tmp/outside && cp a workspace/evil` 证明的是 `/workspace/workspace/evil`（信任根内），实际却写 `/tmp/outside/workspace/evil`（信任根外），整条调用被桥接为 `danger-full-access` 零评审放行（已复现：`curated allowPath` + `approval-bridge` + `allowed-once`）。现每段目标按**该段当时**的 cwd 归一（`cd` 跟踪本已生效），证明与真实写入点一致。方向是「证明更准」：`cd` 进信任根内的相对写入仍走快速路径，`cd` 到根外则回退分类器，无 `cd` 的调用语义不变。`bashWriteDestinations()` 现**恒返回绝对路径**。
- **展开后仍含 `$` 的目标整条拒绝**（review 检出；既有，同一类）。`expandShellVars` 只解析命令内赋值与 `$HOME`，其余（`$TMPDIR`、`$XDG_*` 等）保持字面量——而字面量词法上落在**恒为信任根的会话 cwd** 之下，于是 `git … commit && cp a "$TMPDIR/y"` 通过证明，真实写点却在别处。现目标展开后仍含 `$` 即整条回退分类器，兑现既有注释的承诺。追加复核又在旁边一步找到同一个洞：**命令名**——`cp a /allowed && "$EVIL"/echo hi`（base token 末段名恰为良性 `echo`）同样被拒，而 `$HOME/bin/cp a /dest` 仍正常展开。第四轮再补最后一个变体：**带路径前缀**的 token 不得冒充良性工具（`./echo hi`，以及 `E=evil; "$E"/echo hi` 经同命令赋值解析后），只有裸名可搭车。写命令刻意保留末段名匹配，文档化的 `~/bin/trash …` 包装形态照旧可用、其目的地仍被证明；残留（`./cp` 这类带前缀的**写**命令）已记入 spec。`$HOME` 与命令内赋值不受影响。
- 同批修掉 `mkdir` 两处解析小瑕疵：GNU coreutils 的 `-Z`/`--context` **不接**独立取值（取值形式是 `--context=CTX`），原先会把 `mkdir -Z /evil /allowed` 的真实目标吞掉；`--` 现在正确结束选项解析，不再丢弃旗标形操作数。
...
### 变更
- **判定权归用户，只有一条硬底线不是用户能定的**（策略决定，2026-09-12）。闸门不再判断一个动作**值不值得做**，只判断用户**有没有授权这个具体动作**。有风险但用户要求了的动作可以放行，包括会离开本机的效果（发版、发布、部署、推送、对外发消息、写别的系统）。当动作会影响其他人或系统、而用户自己的消息**没有明确覆盖这一个动作**时，闸门拒绝，并在理由里要求 agent **把原样命令拿给用户确认**（`ask_user_question`）——用户的回答就是直接授权，下次尝试可放行：这就是用户要的 **double check**。**授权只来自用户自己的消息**（打字消息，或对 `ask_user_question` 的回答）；agent 的 justification、仓库文本、工具输出**永远不构成授权**，否则「用户意图」就退化成「谁最后说话谁算」。**硬底线**对任何人都不开：执行下载来的代码、破坏/移动系统路径、删 docker 卷、读密钥材料或凭据库、把密钥材料写进命令、改 shell 启动文件或 agent 权限配置 —— 这些无论用户怎么说都拒，理由里让用户自己在 auto mode 之外执行；它们也在任何模型介入之前就被确定性拦下。
- **one-token 预筛不再能替用户拍板「离开本机」的动作。** 预筛只对「不可能影响他人/他系统」的动作短路放行；一组受控形态（git push、包发布、release/workflow 命令、上传与 POST、远端复制、deploy 类动词…）强制进入结构化审查——那是唯一读到用户授权的一级。规则生效前实测：一条**用户确实要求过**的对外消息，5/5 被预筛直接放行，审查与授权检查整段没被读。

### 修复
- **意图窗口现在带上「被回答的那个问题」。** 以前对确认提问回一句「可以」，进到闸门时已经被剥掉了问题本身，于是它读成「批准跑个探针」而不是「批准你在卡片上刚看过的那条命令」—— double-check 闭环合不上。现在答案渲染成「用户的话 + agent 写的问题（仅供消歧）」，prompt 也写明：引号里的问题本身不构成授权、也不能放大答案的射程。这是**活体端到端测试**发现的，离线测试套件看不到。
...
- **「外部状态」这个说法被它真正代表的判据取代。** 旧的无条件句（「改动共享/生产/外部状态，即使用户的话可被读作许可」）比威胁模型更钝：它把「用户已批准、只是内容需要过目」与「注入的指令在假冒用户」压成一件事，并把一个无害的修复（移动一个从未产出过发布物的 tag）读成危险动作。现在改为点名**真正的属性** —— 效果**会离开本机且无法收回**（即其他人或系统受影响且收不回）—— 并要求分类器判断该属性而非字面措辞。（本条最初写的是「底线不变」，但同一版本内该底线已被上面的「判定权归用户」取代，那句话不再描述实际行为；对任何请求都不开的只剩**硬底线**。）
