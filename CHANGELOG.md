...
### Added
- **The `auto-mode:allowlist` system-prompt section now states the escalation rule.** For a target inside an `allowPath` that lies outside the workspace, the agent is told to request `danger-full-access` on the **first attempt** — the approval bridge grants it with no review — instead of trying bare first and escalating after the sandbox rejects it.
- `npm run test:pathtrust`: an offline end-to-end check of this behaviour (no API key needed).

### Fixed
- **The circuit-breaker hint no longer reads as "escalation always needs a human".** It now says the human is asked **because auto mode is paused**, and that in normal operation allowlisted escalation is auto-approved with zero review.

### Security
- **Composite commands no longer earn allowPath trust on a partial proof.** `mkdir` counts as a write segment; relative destinations resolve against the cwd in effect at each segment; a destination or command name that still contains an unresolved `$` is refused; and a path-prefixed name can no longer stand in for a read-only utility. Anything that cannot be proved now falls back to the classifier instead of being auto-trusted.

### Changed
- `bashWriteDestinations()` always returns absolute paths.
...
### 新增
- **`auto-mode:allowlist` 系统提示段现在写明提权规则**：目标在白名单内、又要写 workspace 外时，**首次尝试就带 `danger-full-access`**（approval 桥接零评审放行），不要先裸跑、等沙箱拒绝后再升级。
- `npm run test:pathtrust`：该行为的 offline 端到端检查（无需密钥）。

### 修复
- **熔断器提示不再被读成「提权永远要人工」**：现在说明走人工是**因为 auto 已暂停**，正常状态下白名单内提权是零评审自动放行。

### 安全
- **组合命令不再凭「部分证明」拿到 allowPath 信任**：`mkdir` 计入写段；相对目标按各段当时的 cwd 解析；目标或命令名展开后仍含未解析 `$` 一律拒绝；带路径前缀的名字不能再冒充只读工具。凡证明不了的组合命令，改为回退分类器而非自动信任。

### 变更
- `bashWriteDestinations()` 现在恒返回绝对路径。
