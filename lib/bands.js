...
const ALT_COMMAND_KEYS = ['script', 'cmd', 'shell_command'];
/**
 ...
 */
export const MACHINE_LEAVING_SHAPES = [
    /\bgit\s+push\b/i,
...
export function looksMachineLeaving(text) {
    if (!text)
        return false;
 ...
 */
export function bashCommandOf(args, toolName) {
    ...
    if (typeof args === 'string') {
        try {
            ...
            return obj && typeof obj === 'object' ? pick(obj) : '';
        }
        catch {
            return args;
        }
...
function collectSegmentDestinations(cmd, vars, dests, cwd) {
    ...
    for (const rawSeg of segments) {
        ...
        if (!BASH_WRITE_COMMANDS.has(base)) {
            ...
            if (first === base && BENIGN_UTILITY_COMMANDS.has(base))
                continue;
