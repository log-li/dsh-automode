...
import { classifyBand, compileRegex, bashCommandOf } from './bands.js';
...
import { classifyFailureCategory, classifyTwoStage, renderTranscript, renderUserIntent, resolveRoute, restoreToolCallArgs, truncateToChars, } from './classifier.js';
...
async function decideAuto(ctx, config, req, next, cache, breaker, bridge, logger) {
    ...
    const input = promptInputOf({ toolName, reason, userIntent, command: commandText || undefined }, softAllowRules, softDenyRules, environmentFacts);
