...
// ---- Helpers ----
export function isAuto(session, ctx) {
...
function writeAutoModeKnobs(ctx, session) {
    if (!hasLegacyPermissionFolds())
        throw new Error('dsh-automode: permissionPresets.set is required on this host');
...
export function apply(ctx, rawConfig) {
    ...
    ctx.on('approval/request', (req, next) => {
        try {
            if (!isAuto(req.agent.session, ctx))
                return next();
        }
        catch (error) {
