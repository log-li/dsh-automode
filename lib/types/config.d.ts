...
export declare const DEFAULT_DENY: readonly ["curl\\s+[^|]*\\|\\s*(?:ba)?sh", "wget\\s+[^|]*\\|\\s*(?:ba)?sh", "-----BEGIN[A-Z ]*PRIVATE KEY-----", "AWS_SECRET", "authorized_keys", "\\.aws/credentials", "(?:^|[:\\s;&|(\"'])(?:trash|mv)\\s+/(?:etc|usr|bin|sbin|var|System|Library|private/etc)(?:/|\\s|$)", "docker\\s+(volume\\s+rm|system\\s+prune|container\\s+prune)", "(?<![a-zA-Z0-9?.])\\.ssh/", "(?<![a-zA-Z0-9?.])\\.zshrc", "(?<![a-zA-Z0-9?.])\\.bashrc", "(?<![a-zA-Z0-9?.])\\.bash_profile", "(?<![a-zA-Z0-9?.])\\.profile\\b", "(?<![a-zA-Z0-9?.])\\.claude/settings", "id_rsa|id_ed25519", "(?<![a-zA-Z0-9?.])\\.pem\\b", "(?<![a-zA-Z0-9?.])\\.env\\b", "(?<![a-zA-Z0-9])(?:\\.credentials|credentials\\.(?:json|ya?ml))\\b"];
