...
- **复合命令里「取不到目标」的写命令段仍搭车**（2026-09-16 独立 review 检出，**未修复**）：
  ...
  - **影响面**：仅「复合命令 + 某段写命令取不到目标 + 另有段落给出落在白名单内的目标」这一组合；单命令、以及任一段是未知/副作用命令的情形都不受影响（后者本就回退）。**用户决定（2026-09-16）：先不修，保持已登记**（需「复合命令 + 提权请求」两个条件才触达；修法需先补「按子命令判定只读」的表）。
...
- **`DEFAULT_ALLOW` 里 `mkdir *` / `trash *` 这类前缀 glob 的**真实作用面**（2026-09-16 记录；**措辞已按实测更正**，**未改默认值**）：allow 频带对**非复合**命令按前缀匹配，命中即让 **pre-execute 门**放行（默认表含 `mkdir *`、`echo *`、`trash *`、`ls*`、`date*` 等，deny 频带仍最先跑）。
  - **实测更正**（此前本条写成「不经 allowPath 也不经分类器」，不准确）：allow 频带只跳过**门**这一层，**不跳过 approval 瀑布**——`decideAuto` 不看 allow 频带。复现：`mkdir -p ~/Documents/newdir` + `danger-full-access` → 门记 `matched allow glob /^mkdir .*$/` 并 `NEXT`，但 approval 路径仍跑分类器（无路由时 fail-closed `rejected`）。
  - **因此真实含义**是：需要**提权**的这类单命令仍会被分类器看到；只有**不需要提权**（沙箱自己就放行）时才完全不审查——而那与 in-tree / 沙箱自身的信任面重合。**残余面**仅是「宽前缀（任意参数）换掉了门这一层」，不含静默越权。改默认表属「参数/行为默认值」变更，需用户确认后再动。**用户决定（2026-09-16）：保持现状**（不收紧 `DEFAULT_ALLOW`）。

## 项目治理规范（对齐行业惯例，2026-09-12）

> **面向 agent 的入口文档是仓库根 `AGENTS.md`**（2026-09-23 新增）：它把本节的治理闸门、
> 硬约束与发布流程压缩成随仓库走的常驻指引。**两者冲突时以本 spec 为准**——spec 是设计真相，
> AGENTS.md 是它的操作摘要。
...
**E2E 门禁（commit 前，且必须独立实例，2026-09-23 用户明确）**
- **顺序固定**：`写代码 → 独立模型家族 review → 按 review 修正 → 端到端验证（绿）→ commit → push`。**E2E 没过不许 commit**——smoke 全绿只证明「你断言到的那一层」是对的，不等于改动可用。
- **E2E 的定义**：在**真实运行环境**里把插件跑起来、走一遍真实路径、**读回真实产物**（会话日志 / `decisions.jsonl` / 模型可见的完整输出文本）。只跑 mock/fixture 的 smoke 不算 E2E；「跑了真实代码路径的离线探针」也不算真机 E2E，**不许把它说成真机验证**。
- **必须独立实例**：**不要用当前在用的实例**（正在服务的 `web` profile、承载会话的运行时）验证——结果不可信（很可能加载的是旧版，误判为「验证通过」）且会污染正在做的事。做法：`DSH_HOME=/tmp/<隔离目录>` + 独立 profile（bundle 列表 + 独立 `cordis.patch.yml`）+ 独立数据目录，用完清理。
...
- **落地动作状态（2026-09-26 收尾）**：
  - ✅ **在用 web profile 已应用**（用户确认后执行）：`~/.dsh/profiles/web/node_modules/@deepseek-ai/{dsh-permission-presets/lib/index.js, dsh-client-ui-permission-presets/lib/client.js}` 与全局 CLI 树同两文件均已打上（marker 各 1；两侧 `.pre-dsh-automode-icon.bak` 备份就位；二次运行报 `already patched`；两份文件 `node --check` 通过）。**重启 `dsh web` 后闪电生效**（宿主半边启动时读取；客户端半边刷新页面即可）。回滚 = 把 `.bak` 抄回这两个文件。
  - ✅ **postinstall 钩子已同步**（用户明确授权后执行，2026-09-26）：`~/.dsh/apply-dsh-patches.sh` 改为跑**插件随包副本**（新增 `run_patch_file()`，保留名称版 `run_patch()` 供将来用），旧副本 `~/.dsh/patches/dsh-permission-preset-icon.mjs` 已 `~/bin/trash` 退役（`~/.dsh/patches/` 现为空目录）；原钩子备份在 `/tmp/icon-patch-artifacts/apply-dsh-patches.sh.prev.<时间戳>`。**钩子端到端自检通过**：在裸 `PATH=/usr/bin:/bin` 下输出四处 `already patched` + 一行 `skipped (older DSH line)`、退出码 0。
  - 顺带修掉钩子自身的脆弱性：原脚本直接调 `node`，在最小 PATH 下会 `node: command not found` 并被 WARN 文案**误报成「锚点未命中」**（本轮实测；`npm` 跑 postinstall 时会把 node 目录前置到 PATH，所以线上一直没暴露）。现在先 `command -v node`，失败再兜底 `/opt/homebrew/bin/node`、`/usr/local/bin/node`、`~/.npm-global/bin/node`，都找不到才 SKIP 并说明原因。
...
- **端到端验证（v0.15.3 新增，`npm run test:pathtrust`）**：`scripts/e2e-path-trust.mjs` 针对**编译产物** + 宿主 mock 跑变更路径的 E2E——① **完整打印**模型可见的 `auto-mode:allowlist` 全文（审计「模型实际看到的那一面」，不只查改动的那句）；② 跑通白名单提权快路径（门 → `curated allowPath` → `approval-bridge` → `allowed-once`，零分类器）与审计留痕；③ 断言四处收紧**都不**被自动放行；④ 断言非 auto 会话拿到空段（门控）。**边界**：这是**逻辑层 E2E**（mock host）；真机结果见下一条。
- **真机 E2E（2026-09-16，用户重启 `dsh web` 后实测；宿主日志 `boot dsh-automode v0.15.3 active` 证实已加载新版）**：
  1. **指引注入**：新版 `auto-mode:allowlist` 全文出现在真实会话的系统提示里（重启前对照为旧句）。
  2. **白名单提权零评审**（两条路径各一次）：① 文件工具写 `~/.claude/<probe>` → `pre-execute-fileop(esc=true)` → `curated allowPath` → `approval-bridge` → `decision allowed-once`（~20ms，无分类器、无人工）；② bash 写命令 `~/bin/trash ~/.claude/<probe>` → `bashDests=["…/<probe>"]` → 同一条链（顺带验证 `~` 展开与 trash 包装脚本形态）。
  3. **复合命令收紧（本次修复的核心断言）**：`cp /dev/null ~/.claude/x && mkdir -p ~/Documents` 真机**不再**零评审放行——`pre-execute-bashop` 的 **`bashDests` 已含 mkdir 目标**（`["…/.claude/x","~/Documents"]`）→ `every(...)` 不成立 → **无 `curated allowPath`** → 分类器拒绝，理由可执行（点名「workspace 外建目录 + 用户未授权该提权」，并给出确认路径）。v0.15.3 之前此处只交白名单目标，会被桥接为 `danger-full-access` 零评审放行。
  4. **旁证（策略生效）**：一条把「清理」与「读日志」拼在一起的复合命令（含 `python3`/管道 → 必回退分类器）被正确拒绝并给出 double-check 指引——即「删 workspace 外文件 + 提权」在用户未明确授权时会被拦下。**副作用为零**（被拒调用不执行）；探针文件已用 `trash` 回收。
