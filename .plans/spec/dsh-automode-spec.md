...
- **端到端验证（v0.15.3 新增，`npm run test:pathtrust`）**：`scripts/e2e-path-trust.mjs` 针对**编译产物** + 宿主 mock 跑变更路径的 E2E——① **完整打印**模型可见的 `auto-mode:allowlist` 全文（审计「模型实际看到的那一面」，不只查改动的那句）；② 跑通白名单提权快路径（门 → `curated allowPath` → `approval-bridge` → `allowed-once`，零分类器）与审计留痕；③ 断言四处收紧**都不**被自动放行；④ 断言非 auto 会话拿到空段（门控）。**边界（须诚实声明）**：这是**逻辑层 E2E**（mock host），不等于真机——真机需重启 `dsh web` 后在新会话验证注入文本与真实提权放行；real-model 分类器 E2E 仍是 opt-in 的 `npm run test:e2e`（需 `E2E_BASE_URL`/`E2E_MODEL`/`E2E_API_KEY`）。
