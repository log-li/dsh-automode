...
- **真机 E2E（2026-09-16，用户重启 `dsh web` 后实测；宿主日志 `boot dsh-automode v0.15.3 active` 证实已加载新版）**：
  ...
  3. **复合命令收紧（本次修复的核心断言）**：`cp /dev/null ~/.claude/x && mkdir -p ~/Documents` 真机**不再**零评审放行——`pre-execute-bashop` 的 **`bashDests` 已含 mkdir 目标**（`["…/.claude/x","/Users/logan/Documents"]`）→ `every(...)` 不成立 → **无 `curated allowPath`** → 分类器拒绝，理由可执行（点名「workspace 外建目录 + 用户未授权该提权」，并给出确认路径）。v0.15.3 之前此处只交白名单目标，会被桥接为 `danger-full-access` 零评审放行。
