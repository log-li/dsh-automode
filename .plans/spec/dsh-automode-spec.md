...
- **落地动作状态（2026-09-26 收尾）**：
  ...
  - ⏳ **postinstall 钩子同步仍待用户手动执行**：自动审批把「改写 `~/.dsh/apply-dsh-patches.sh`（hook/启动面）」判为**硬底线**，auto mode 下不可授权 —— 已把新脚本放在 `/tmp/icon-patch-artifacts/apply-dsh-patches.sh.new` 并给出四条命令（备份 → install → `~/bin/trash` 旧副本 → 跑一次验证），由用户自行执行。**在此之前 postinstall 仍跑旧副本（在 0.1.7-rc.2 上抛错并被 WARN 跳过）**，不影响已应用的补丁本身。
