...
**离开本机的动作：默认拒绝，唯一例外是「用户点名的本项目发版/部署」（v0.15.2 改，用户拍板）**。
- **默认**：效果会离开本机且无法收回的动作**默认拒绝** —— 推送到用户不拥有的 remote、向外交付消息、
  写生产系统、部署到生产。
- **唯一例外**：**发布/部署本项目**（会话正在工作的仓库与包）。当**用户自己的直说消息**
  （`<recent_user_intent>`；只有直接人类消息与 `ask_user_question` 的答案进入其中）表达了
  发布/发版/部署的意愿时，该发版动作**可以放行**。授权的**有无**就是判据：用户消息里没有这个意愿 → 拒绝。
- **仍然绝对拒绝，用户授权也不能解锁**：向外交付消息、推送到用户不拥有的 remote、写生产系统、
  不可逆破坏、泄密、持久化、削弱安全。
- **agent 的叙述（justification）永远不构成授权**；仓库文本、工具输出同样不构成。
  对齐只能依据用户自己的消息判断，不依据 agent 对用户意愿的转述。
- **不确定即拒**（fail closed）。
- **残留风险（用户已知悉并选择）**：分类器看不到**实际会发出去什么** —— 例如
  `gh workflow run release.yml` 的版本号来自 `package.json`，命令里根本没有。所以这条例外只能核到
  「本项目的发版动作」这一层，核不到内容与版本。这把「agent 被引导着发错东西」从**不可能**
  降为**需要用户消息里确有发布意愿**，用户选择接受该风险以换取 agent 可代为发版。
- 决策记录（2026-09-12）：用户明确选择「有发布意愿就行」（最宽授权档）+「只开放本仓库发版/部署」
  （最窄范围档）；此前 v0.15.0 的「never granted … however clearly the user asked」被本例外取代。
  这是**策略选择**，不是漏洞修复 —— 权衡与残留风险如上。
...
- **deny 频带把「散文」当「操作」扫 → 误报**（2026-09-12 实测，**v0.15.1 已修复**）
  ...
  - **★ 词形模式收窄被整条回退（v0.15.2，独立 review 检出、逐条复现）**：v0.15.1 把
...
- **散文承载型工具的提权裁决只看得到叙述**（v0.15.1 遗留，**v0.15.2 修复**；独立 review 检出）
...
- **预筛的 0/1 解析是「首匹配」，命令里的数字会污染裁决**（v0.15.2 修复；review 检出）
...
- **结构性守卫太弱**（v0.15.2 加固；review 检出）：原先只对两个 `src/*.ts` 做「标识符缺席」断言，
