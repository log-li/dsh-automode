...
/** Tool names that execute shell text (so a `script`/`cmd` argument IS a command).
 * Deliberately excludes generic verb names (`run`, `exec`, `console`, `terminal`)
 * whose `script`/`cmd` field is far more likely to be data than a command
 * (v0.15.1 review m5). */
const SHELL_TOOL_NAME = /^(?:bash|sh|zsh|fish|ksh|csh|tcsh|pwsh|powershell|cmd|shell|shell_exec)$/i;

/** Argument keys that may carry an executable command (after `command` itself). */
const ALT_COMMAND_KEYS = ['script', 'cmd', 'shell_command'] as const;
 ...
 */
export const MACHINE_LEAVING_SHAPES: readonly RegExp[] = [
  /\bgit\s+push\b/i,
  /\bgit\s+send-email\b/i,
  /\b(?:npm|yarn|pnpm)\s+(?:publish|unpublish|deprecate)\b/i,
  /\b(?:lerna|cargo|twine|gem|poetry)\s+(?:publish|upload|push)\b/i,
  /\bgh\s+(?:release|workflow|api|repo|pr|secret|run|gist)\b/i,
  /\b(?:glab|hub)\b/i,
  /\b(?:docker|podman)\s+(?:compose\s+)?push\b/i,
  /\b(?:docker|podman)\s+compose\s+(?:up|down)\b/i,
  /\b(?:kubectl|helm)\s+(?:apply|delete|upgrade|rollout|create|patch|install|uninstall)\b/i,
  /\bterraform\s+(?:apply|destroy)\b/i,
  /\b(?:aws|gcloud|az)\s+\S+\s+(?:cp|sync|create|delete|update|deploy|put-object|send)\b/i,
  /\b(?:s3cmd|rclone)\b/i,
  /\bcurl\b[^\n]*(?:-X\s*(?:POST|PUT|PATCH|DELETE)|--data\b|--upload-file|-T\s|-F\s|--form\b|\s-K\b|--config\b)/i,
  /\bwget\b[^\n]*(?:--post-data|--post-file|--method=POST)/i,
  /\b(?:scp|sftp)\b/i,
  /\brsync\b[^\n]*\s[^\s:]+:/,
  // Any ssh invocation (a flag may precede the destination), plus the raw-socket,
  // mail and inline-script families — all of them ship data off this machine
  // (v0.15.1 review M2: `ssh -p 2222 h`, `nc`, `mailx`, `python3 -c …` all missed).
  /\bssh\b/i,
  /\b(?:mosh|socat|telnet)\b/i,
  /\b(?:nc|ncat|netcat)\b/i,
  /\/dev\/(?:tcp|udp)\//i,
  /\b(?:mail|mailx|ssmtp|sendmail|msmtp|mutt)\b/i,
  /\bopenssl\s+s_client\b/i,
  /\bftp\b/i,
  /\b(?:python[0-9.]*|node|ruby|perl|php)\s+(?:-c|-e|-r)\b/i,
  /\b(?:vercel|heroku|netlify|fly|flyctl|surge)\b/i,
  /\bdeploy\b/i,
];

/** Whether an action's text can affect other people or systems. */
export function looksMachineLeaving(text: string): boolean {
  if (!text) return false;
  return MACHINE_LEAVING_SHAPES.some((re) => re.test(text));
}
/**
 ...
 */
export function bashCommandOf(args: unknown, toolName?: string): string {
  const altKeys = toolName && SHELL_TOOL_NAME.test(toolName) ? ALT_COMMAND_KEYS : [];
  const pick = (obj: Record<string, unknown>): string => {
    if (typeof obj.command === 'string') return obj.command;
    for (const key of altKeys) if (typeof obj[key] === 'string' && obj[key]) return obj[key] as string;
    return '';
  };
  if (typeof args === 'string') {
    try {
      const obj = JSON.parse(args);
      return obj && typeof obj === 'object' ? pick(obj as Record<string, unknown>) : '';
    } catch {
      return args;
    }
  }
  if (args && typeof args === 'object') return pick(args as Record<string, unknown>);
  return '';
}
 ...
 */
export const SUBJECT_ONLY_DENY_SOURCES: ReadonlySet<string> = new Set([
  'AWS_SECRET',
  'authorized_keys',
  '\\.aws/credentials',
  '(?<![a-zA-Z0-9?.])\\.ssh/',
...
export function isProseCarrier(toolName: string, commandText: string): boolean {
 ...
 */
export function proseSafeDenyPatterns(patterns: readonly RegExp[]): RegExp[] {
